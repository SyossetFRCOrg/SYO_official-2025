    ...
    for (int cameraIndex = 0; cameraIndex < io.length; cameraIndex++) {
      ...
      for (var observation : inputs[cameraIndex].poseObservations) {
        ...
        if (rejectPose) {
          ...
          // If the pose is rejected, skip the rest of the loop
          continue;
        } else {
        ...
        // Apply final multipliers based on MegaTag alg and camera specs
        if (observation.type() == PoseObservationType.MEGATAG_2) {
          linearstdDevFactor *= linearStdDevMegatag2Factor;
          thetastdDevFactor *= angularStdDevMegatag2Factor;
        }
        if (cameraIndex < cameraStdDevFactors.length) {
          linearstdDevFactor *= cameraStdDevFactors[cameraIndex];
          thetastdDevFactor *= cameraStdDevFactors[cameraIndex];
        }
